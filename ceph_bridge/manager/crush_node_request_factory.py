    ...
    def update(self, node_id, attributes):
        ...
        if name != current_node[
            ...
            if parent is not None:
                commands.append(move_bucket(
                    name, parent['name'], parent['type']))

        to_remove = [item for item in current_node[
            'items'] if item not in items]
    ...
    if node is not None:
        if node['items']:
            raise BucketNotEmptyError(
                'Cannot delete a bucket that still contains items')
    ...
    return (
        ...
        ('config-key put',
         {'key': 'daemon-private/osd.{id}/v1/calamari/'
          'osd_crush_location'.format(id=osd_id),
...
def move_bucket(name, parent_name, parent_type):
    return ('osd crush move',
            {'name': name,
             'args': [
                 "{type}={name}".format(type=parent_type, name=parent_name)
             ],
             }
            )
