...
class OsdMapModifyingRequest(RadosRequest):
    ...
    def complete_jid(self, result):
        # My remote work is done, record the version of the map
        # that I will wait for and start waiting for it.
        self.jid = None
        self.result = result
...
class PgCreatingRequest(OsdMapModifyingRequest):
    ...
    def status(self):
        if not self.state == \
           self.COMPLETE and not self._pg_progress.is_complete():
    ...
    def on_map(self, sync_type, sync_object):
        ...
        if self._phase == self.PG_MAP_WAIT:
            ...
            if pool['pg_num'] != self._pg_progress.expected_count():
                self.set_error(
                    "PG creation failed (check that there"
